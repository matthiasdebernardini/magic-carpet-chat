...
const cnt = { open: 0, res: 0, routed: 0 };
NODES.forEach((n) => (n.cond || []).map(Q).forEach((c) => (c.r ? cnt.res++ : c.to ? cnt.routed++ : cnt.open++)));
...
function buildSystemMd() {
  ...
  out.push(META.intro, '');
  out.push(`_Question status: **${cnt.open} open · ${cnt.routed} routed · ${cnt.res} resolved**._`, '');
  ...
  out.push('## Flows (representative packets)', '', META.flowsNote || 'Payload shapes are what the design implies, not measured traffic.', '');
  ...
  out.push('## Questions — index', '', 'Reference by ID. ✓ resolved (with date) · → routed to a named next step · otherwise open.', '');
  index.forEach(([id, code, c]) => out.push(c.r ? `- ~~**${id}**~~ (${code}) ✓ ${md(c.r)}` : c.to ? `- **${id}** (${code}) ${md(c.q)} → routed: _${md(c.to)}_` : `- **${id}** (${code}) ${md(c.q)}`));
  out.push('');
